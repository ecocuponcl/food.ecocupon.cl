...
class RuleEvaluator:
    ...
    def _run_check(self, check: str, intent: Dict, output: Dict) -> bool:
        """Run a check expression. Supports &&, ||, → operators."""
        if not check:
            return True
        ...
        if " → " in check or " -> " in check:
            ...
            if self._eval_compound(condition, intent, output):
                if len(parts) > 1:
                    return self._eval_compound(parts[1].strip(), intent, output)
            return True

        return self._eval_compound(check, intent, output)

    def _eval_compound(self, expr: str, intent: Dict, output: Dict) -> bool:
        """Evaluate compound expressions with && and ||."""
        # Handle && (AND)
        if " && " in expr:
            parts = expr.split(" && ")
            return all(self._eval_simple(p.strip(), intent, output) for p in parts)
        # Handle || (OR)
        if " || " in expr:
            parts = expr.split(" || ")
            return any(self._eval_simple(p.strip(), intent, output) for p in parts)
        return self._eval_simple(expr, intent, output)
